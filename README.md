...
- 内存回收机制主要体现在两个方面：
  - **删除到达过期时间的键对象**。
    ...
    - 定时任务删除：`Redis`内部维护一个定时任务，默认每`s`运行`10`次。定时任务删除过期键采用了自适应算法。流程：
...
public class LRUCache{
    ...
    public LRUCache(int capacity){
        ...
        map = new LinkedHashMap<Integer,Integer>(capacity,0.75f,trye){
            @Override
            protected boolean removeEldestEntry(Map.Entry eldest){
                return size() > capacity;//容量大于capacity就删除
            }
        };
    }
